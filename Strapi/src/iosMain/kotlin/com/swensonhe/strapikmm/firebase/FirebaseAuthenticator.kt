...
actual class FirebaseAuthenticator actual constructor(
    ...
    actual fun signIn(
        email: String,
        password: String
    ): CommonFlow<DataState<String>> = flow<DataState<String>>{
    ...
    actual fun signOut(): CommonFlow<DataState<Unit>> = flow<DataState<Unit>> {
    ...
    actual fun signIn(authCredential: AuthCredential) = flow<DataState<String>> {
        executeCatching<String>({
            emit(DataState.loading())
    ...
    actual fun signIn(token: String) = flow<DataState<String>> {
        executeCatching<String>({
            emit(DataState.loading())
